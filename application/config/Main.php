...
class Main extends CI_Controller {
	...
	public function edit_vis($id){
		...
		if (isset($dimensions->indicators)) {
			...
			$link = 'https://hmis.mohz.go.tz/api/analytics.json?dimension=dx:'.$dx.'&dimension=pe:LAST_YEAR';
	...
	public function delete_vis($id){
		...
		// var_dump('dvis',$dvis,'dconf',$dconf,'ids',$ids);
	...
	public function create_y_data_by_dimension($api_data,$x_data_id,$visual_response){
		...
		// var_dump("the x_data here",$x_data_id);
		...
		$data_indi['dataSorting']=$data_sorting_enabled;
	...
	function get_yearly_series_data($response,$visual,$orgUnit,$indicators){
		...
		$vis_details=$this->get_link_data($link);



		$vis_details=json_decode($vis_details,true);
		...
		$api_response=json_decode($this->get_link_data($link),true)['rows'];
	...
	function get_link_data($link){
		...
		$pass= base64_decode($_SESSION['userPass']);
	...
	// var_dump("Here outside", json_encode($countred));
	...
	public function get_msdqi_filter($column_dimension,$api_data,$orgUnit,$period_arr){
		...
		if(array_key_exists($indicator,$api_data['metaData']['items'])){
			...
			// var_dump("the Ou array",	$ou_array,'column_dimension',$column_dimension);



		// foreach($og_units as $ou){
	...
	public function get_table_data($api_data,$visual_response){
		...
		foreach($dimension_id_array as $dimension_id){
			...
			$y_data_x[$row_dimensions[0]]=$row[array_search($row_dimensions[0],$dimension_array)];
	...
	function get_analytics_api($indicator_id, $year,$level)
	{
		...
		CURLOPT_RETURNTRANSFER => 1,
		CURLOPT_USERPWD => $uname.':'.$pass ,
		CURLOPT_URL => 'https://hmis.mohz.go.tz/api/analytics?dimension=dx:'.$indicator_id.';'.$indicator_id.'&dimension=pe:'.$year.'&&dimension=ou:'.$level.'',
		CURLOPT_USERAGENT => 'DHIS api'
		]);
		$responsee = curl_exec($curl);
		curl_close($curl);
	...
	{
		$val = $this->input->post('val');
		$link = 'https://hmis.mohz.go.tz/api/indicators.json?filter=indicatorGroups.id:eq:'.$val.'';
	...
	function get_visualization_data33($startDate,$endDate, $indicator, $organizationUnit){
		$curl = curl_init();
		$uname=$_SESSION['userName'];
		$pass= base64_decode($_SESSION['userPass']);
		...
		CURLOPT_USERPWD => $uname.':'.$pass,
		//CURLOPT_URL =>'https://hmis.mohz.go.tz/api/visualizations/'.$visulId.'.json?;paging=false',
		CURLOPT_URL =>'https://hmis.mohz.go.tz/api/analytics?dimension=dx:'.$indicator.'&dimension=ou:'.$organizationUnit.'&startDate='.$startDate.'&endDate='.$endDate.'',
		CURLOPT_USERAGENT => 'DHIS api'
		]);
		$responsee = curl_exec($curl);
		return $responsee;
	...
	function get_trimmed_period($relative, $fixed){
	...
	function get_filtered_organization_unit($selected_clinics, $selected_districs, $selected_regions, $orggroups, $orglevels){
